...
import { moneyFormat, postInfo, printClientOrder } from '../../../../utils/functions';
...
export function ClientOrderPreview(){
    ...
    const total = useMemo(() => {
        return services.reduce((acc, el) => {
            // Convertimos a número (parseFloat o el operador +)
            const units = parseFloat(el.units) || 0;
            const value = parseFloat(el.value) || 0;
            
            return acc + (units * value);
        }, 0);
    }, [services]);
    ...
    useEffect(()=>{
        ...
        if(docInfo.instance_id != undefined){
            setInstaceId(docInfo.instance_id);
        }
        getAttachedServices();
        getThirdParties()
    },[docInfo])
    ...
    if(stage == 0)return(
        <div className="SelectCashRecipt">
            ...
            <SearchinList title={'Recibo de caja'} placeHolder={'Seleccione el recibo de caja a imprimir'} action={handleSelectDoc} list={clientOrders}/>
        </div>
    )
    if(stage == 1)return(
        <>
        ...
        <div className="CashReciptDesign" style={{
            ...
            <span style={{margin:"2mm 0",width:"100%",borderBottom:"dashed .5mm #000"}}></span>
            <div style={{display:"flex",border:"solid .5mm #000",paddingBottom:"1mm",fontSize:"10px",fontWeight:"bold",padding:"1mm",marginBottom:"3mm"}}>
                <span style={{width:"50%"}}>Item</span>
                <span style={{textAlign:"center",width:"10%", margin:"auto"}}>Unidades</span>
                <span style={{margin:"auto",marginRight:"0"}}>Valor</span>
            </div>
            <div style={{
                display:"flex",
                flexDirection:"column",
                ...
                {services.map((element,index)=>(
                    <div key={index} style={{display:"flex",fontSize:"10px", borderBottom:"dashed .5mm #000",paddingBottom:"2mm"}}>
                        <span style={{width:"50%"}}>{element.name}</span>
                        <span style={{textAlign:"center",width:"10%", margin:"auto"}}>{element.units}</span>
                        <strong style={{margin:"auto",marginRight:"0"}}>{moneyFormat(element.value)}</strong>
                    </div>
                ))}
            </div>
            <div style={{display:"flex",fontSize:"12px",marginTop:"2mm"}}>
                <span>TOTAL:</span>
