...
export const LightChart = ({ title, doc_type, type }) => {
  ...
  const loadData = async () => {
    setLoading(true);
    try {
      const info = { title, doc_type, type }; // <- se construye dinámicamente
      const response = await postInfo(`/getDocumentData?period=${period}`, info);

      // Estructura esperada: [true, [ { total_registros: X } ]]
      if (response[0] && Array.isArray(response[1])) {
        // Convertimos el resultado en un formato apto para Recharts
        const chartData = response[1].map((item, index) => ({
