...
export function ProcessStatusAlert({instance_id,reloadFun}){
    ...
    const advanceNextStep = async()=>{
        ...
        let res = await postInfo('/process/nextProcessStep',{
            ...
            description,
            // Acceso externo: el backend resuelve el responsable interno como autor.
    ...
    const cancellProcess = async()=>{
        let res = await postInfo('/process/updateProcessInstanceStatus',{
            ...
            id:info.id,
            status:'cancelled',
            // Acceso externo: el backend resuelve el responsable interno.
            external_access:true,
            company_key:appInfo.company_key,
            access_key:userInfo.user_key
        });
    ...
    useEffect(()=>{
        if(nextStepData == undefined) return;
        console.log('Info next',nextStepData);
        console.log('Usuario',(userInfo.role))
        console.log('Usuario info',userInfo)
        console.log('Habilitado',!nextStepData.required_roll.includes(parseInt(userInfo.role)))
    },[nextStepData])
