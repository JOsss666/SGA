...
export function AppInfoProvider({children}){
    ...
    const routesApp = [
        {text:'Inicio',path:'',icon:<img src='https://cdnmain.sga360.co/static/LogoInicio1_nsuzaj.webp' />,action:handleNavigate},
        {text:'Informes',path:'reports',icon:<img src='https://cdnmain.sga360.co/static/InformesLogo1_iisxav.webp'/>,action:handleNavigate},
            {text:'Informe de procesos',path:'reports/Processes',icon:<img src='https://cdnmain.sga360.co/static/InformesLogo1_iisxav.webp'/>,action:handleNavigate},
    ...
    const getAppData = async()=>{
        ...
        try{
            let appI = await postInfo('/externalAccess/getCompanyInfo',{
                company_key: params.company_key,
                access_key: params.user_key
            });
            if(appI?.status === 'OK'){
                setAppInfo(appI.data);
                setAppConfig(appI.data.config);
            ...
            if(!location.pathname.startsWith('/preview/')){
                ...
                let userI = await postInfo('/externalAccess/getUserInfo',{
                    company_key: params.company_key,
                    access_key: params.user_key
                });
                console.log(userI);
                if(userI?.status === 'OK' && userI.data.user_session == 1){
                    setUserInfo(userI.data)
                    if(userI.data.config != undefined){
                        setUserConfig(userI.data.config)
                    }
                }else{
                    handleRedirect();
                }
            ...
            console.error('No fue posible validar el acceso externo', error);
            handleRedirect();
        }finally{
