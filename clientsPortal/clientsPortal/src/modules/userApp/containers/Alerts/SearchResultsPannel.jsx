...
export function SearchResultsPannel({searchValue}){
    ...
    // requirements
    const {appInfo,userInfo,userConfig,appConfig,routesApp} = useAppInfo();
    ...
    const searchF = async(search)=>{
        ...
        const nextResults = Array.isArray(res)
            ? res.filter(category => !['accounts', 'transactions'].includes(category.title))
            : [];
    ...
    // El contexto es la única fuente de secciones habilitadas para este portal.
    const navigationCatalog = (routesApp ?? []).map(route => ({
        ...route,
        group: route.group ?? 'Secciones'
    }));
            ...
            <div className="bodyResultsPannel">
                ...
                {!loading && !hasSearched && (
                    <div className="emptyState">
                        ...
                        <p>Busca secciones, terceros, documentos y procesos</p>
                    </div>
                )}
