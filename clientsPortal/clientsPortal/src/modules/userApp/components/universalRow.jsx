...
function UniversalRowComponent({
    ...
    const extraClassName = typeof getRowClassName === 'function' ? (getRowClassName(info) ?? '') : '';
    ...
    return (
        <div
            className={`universalRow${selected ? ' selected' : ''}${disabled ? ' disabled' : ''}${clickable ? ' clickable' : ''}${extraClassName ? ` ${extraClassName}` : ''}`}
            role="row"
            aria-rowindex={index + 2}
            aria-selected={selected}
            aria-disabled={disabled}
            onClick={clickable && !disabled ? () => onRowClick(info) : undefined}
        >
