...
export function TableTransactions({setLoading,setTotalBill,docInfo,aplyTransactions,type,reloadFun,searchValue}){
    ...
    const saveMovement = async()=>{
        ...
        if(typeof res[1] == 'number'){
            addNotification({
                type:'aproved',
                title:`CI#${res[1]} creada`,
                description:`El consumo de inventario (CI#${res[1]} se a añadido correctamente a la orden de producción (OP#${docInfo.op_id}))`
            })
        }else{
            addNotification({
                type:'error',
                title:`Error al crear consumo de inventario.`,
                description:`${res[1][1]}.`
            })
        }
