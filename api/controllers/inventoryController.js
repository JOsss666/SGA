...
inventoryController.getProducts = (req,res)=>{
    ...
    req.on('end',async()=>{
        let info = JSON.parse(data)
        if(info.tree){
            sentence = `SELECT * FROM "Inventory".products WHERE category_id = ? AND subCategory_id = ? ;`
            ...
            if(info.totalStocks){
                ...
                sentence = `SELECT * FROM "Inventory".products WHERE company_id = ?`;
            }
...
inventoryController.createProduct = (req,res)=>{
    ...
    req.on('end',async()=>{
        let info = JSON.parse(data);
        console.log(info)
        let sentence = `INSERT INTO "Inventory".products (company_id,product_code,product_name,supplier_id,category_id,stock,units,product_description) VALUES(?,?,?,?,?,?,?,?);`
...
inventoryController.updateProductList = (req,res)=>{
    ...
    req.on('end',async()=>{
        ...
        if(info.price_id == undefined){
            let sentence = `INSERT INTO "Inventory".pricesProducts (company_id,store_id,list_id,product_id,unit_value,price_state) VALUES (?,?,?,?,?,?); `
...
inventoryController.getPriceStock = (req,res)=>{
    ...
    req.on('end',async()=>{
        let info = JSON.parse(data)
        console.log(info);
        let sentence = `SELECT products.*, stocks.* FROM "Inventory".stocks LEFT JOIN products ON products.product_id = stocks.product_id WHERE stocks.product_id = ? AND stocks.list_id = ? ;`;
    ...
    req.on('end',async()=>{
        ...
        if(preConsul){
            ...
            if(prevCons[0]){
                ...
                let s1 = `INSERT INTO "Inventory".pricesProducts (company_id,store_id,list_id,product_id,total_cost,unit_cost,price_state) VALUES (?,?,?,?,?,?,?);`;
...
inventoryController.newDeparture = (req,res)=>{
    ...
    req.on('end',async()=>{
        ...
        if(prevIn){
            let s2 = `SELECT pricesProducts.* , stocks.stock FROM "Inventory".pricesProducts LEFT JOIN stocks ON pricesProducts.list_id = stocks.list_id AND pricesProducts.product_id = stocks.product_id WHERE pricesProducts.list_id = ? AND stocks.stock_id = ? ;`
