...
/**
 * Informe operativo de NEXO 360.
 *
 * Entrega únicamente órdenes de cliente reales de la compañía. El identificador
 * de cada fila es el ID de `Ecosystem.documents`, que es el mismo que consume
 * la delegación masiva mediante `allowedIds`.
 */
processController.getNexoProcessAdministrationReport = (req, res) => {
    let body = '';
    ...
    req.on('end', async () => {
        try {
            ...
            const companyId = String(info.company_id ?? '');
            if(!/^[1-9]\d*$/.test(companyId)) {
                res.writeHead(400, {'Content-Type':'application/json'});
                res.end(JSON.stringify({error:'company_id debe ser un ID válido.'}));
                return;
            }
            ...
            `;
            const consulta = await useDataBase(sentence, [companyId], 1);
            res.writeHead(200, {'Content-Type':'application/json'});
            res.end(JSON.stringify(consulta));
        } catch (error) {
            console.error('Error al cargar informe NEXO 360:', error);
            res.writeHead(500, {'Content-Type':'application/json'});
            res.end(JSON.stringify({error:'No fue posible cargar el informe NEXO 360.'}));
        }
    });
    req.on('error', () => {
        res.writeHead(500, {'Content-Type':'application/json'});
        res.end(JSON.stringify({error:'No fue posible recibir la solicitud.'}));
    });
}
